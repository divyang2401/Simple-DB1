...
public class HeapPage implements Page {
	...
	public Tuple getTuple(int entryID) {
		int p = tupleLocation(entryID);
		int r = entryCount();
		if(entryID < r )  {
			DataInputStream in = new DataInputStream(new ByteArrayInputStream(data, p, data.length-p));
	...
	public void addTuple(Tuple t) throws DbException {
		...
		{
			throw new DbException("freeSpaceSize is less"); 
		}
		else {
			System.arraycopy(b, 0, data, location, b.length);
			int r = entryCount() + 1;
			//System.out.println(entryCount()+ "test");
			saveEntryCount(r);
			int entryID = entryCount();
			//System.out.println(entryID+ "test");
			saveTupleLocation( entryID, location);
			t.setRecordId(new RecordId(pid,r));
