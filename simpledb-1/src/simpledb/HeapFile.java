...
public class HeapFile implements DbFile {
	...
	public Page readPage(PageId pid) {
		try {
		RandomAccessFile random_file = new RandomAccessFile(getFile(),"r");
		int offset = BufferPool.PAGE_SIZE * pid.pageno();
		byte[] data =new byte[BufferPool.PAGE_SIZE];
		if(offset + BufferPool.PAGE_SIZE > random_file.length()) {
			}
		random_file.seek(offset);
		random_file.readFully(data);
		random_file.close();
